...
func newWGDevice(devAttrs *wgDeviceAttrs, ctx context.Context, wg *sync.WaitGroup) (*wgDevice, error) {
	log.Infof("newWGDevice %s", devAttrs.name)
...
func (dev *wgDevice) upAndAddRoute(dst *net.IPNet) error {
	log.Infof("Up and Add Route %v", dst)
...
func (dev *wgDevice) addPeer(publicEndpoint string, peerPublicKeyRaw string, peerSubnets []net.IPNet) (err error) {
