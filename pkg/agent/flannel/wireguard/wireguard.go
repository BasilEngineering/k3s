...
func New(sm subnet.Manager, extIface *backend.ExternalInterface) (backend.Backend, error) {
	...
	kc, err := newKubeClient(context.Background(), KubeConfig)
	if err != nil {
		return nil, err
	}
	be.kube = kc
...
func (be *WireguardBackend) RegisterNetwork(ctx context.Context, wg *sync.WaitGroup, config *subnet.Config) (backend.Network, error) {
	...
	port := 51820
	envPort, envExists := os.LookupEnv("WIREGUARD_PORT")
	if envExists {
	...
	return newNetwork(be.sm, be.extIface, dev, v6Dev, cfg.Mode, lease, cfg.MTU, be.kube)
}
