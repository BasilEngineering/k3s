...
func newNetwork(sm subnet.Manager, extIface *backend.ExternalInterface, dev, v6Dev *wgDevice, mode Mode, lease *lease.Lease, mtu int, kube *kubeClient) (*network, error) {
	n := &network{
		dev:      dev,
		v6Dev:    v6Dev,
		extIface: extIface,
...
func (n *network) handleSubnetEvents(ctx context.Context, batch []lease.Event) {
	for _, event := range batch {
		nodes, err := n.kube.Nodes(ctx)
		if err != nil {
			log.Errorf("handleSubnetEvents: %v fetch nodes failed: %v", event.Lease.Subnet, err)
		}
		...
		case lease.EventAdded:
			...
			if n.mode == Separate {
				if event.Lease.EnableIPv4 {
					...
					subs := []net.IPNet{*event.Lease.Subnet.ToIPNet()}
					var addRouteTo []*net.IPNet
					log.Infof("Subnet add: %v via %v target=%v current=%v", event.Lease.Subnet, publicEndpoint, target, current)
					if target != nil && current != nil {
						log.Infof("Subnet add: %v via %v target_edge=%v current_cloud=%v", event.Lease.Subnet, publicEndpoint, isNodeEdge(target), isNodeCloud(current))
						if isNodeCloud(current) && isNodeEdge(target) {
							publicEndpoint = ""
					...
					for _, ato := range addRouteTo {
						if err := n.dev.addRoute(ato); err != nil {
							log.Errorf("failed to add ipv4 route to (%s): %v", ato, err)
						}
					}
				}
				...
				if event.Lease.EnableIPv6 {
					...
					subs := []net.IPNet{*event.Lease.IPv6Subnet.ToIPNet()}
					var addRouteTo []*net.IPNet
					if target != nil && current != nil {
						if isNodeCloud(current) && isNodeEdge(target) {
							publicEndpoint = ""
					...
					for _, ato := range addRouteTo {
						if err := n.v6Dev.addRoute(ato); err != nil {
							log.Errorf("failed to add ipv4 route to (%s): %v", ato, err)
						}
					}
				}
